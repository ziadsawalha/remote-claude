...
export function BgTasksView({ sessionId }: { sessionId: string }) {
	...
	if (bgTasks.length === 0) {
		return (
			<div className="text-muted-foreground text-center py-10 text-xs">
				No background tasks
			</div>
		)
	}
	...
	return (
		<div className="h-full overflow-y-auto p-3 sm:p-4 space-y-4">
			...
			<div className="flex gap-3 text-xs">
				{running.length > 0 && (
					<span className="text-emerald-400 font-bold">{running.length} running</span>
				)}
				{completed.length > 0 && (
					<span className="text-muted-foreground">{completed.length} completed</span>
				)}
				{killed.length > 0 && (
					<span className="text-red-400">{killed.length} killed</span>
				)}
			</div>
